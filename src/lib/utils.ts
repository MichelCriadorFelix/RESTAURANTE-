...
// an unrelated place/block number anywhere in the country (this produced a
// real false "1000+km away" delivery-radius rejection for a customer whose
// street genuinely isn't mapped). This instead backs off through
// progressively broader — but still geographically real — queries, and
// never touches the CEP as a bare search string. Worst case it resolves to
// the city center, which for a local delivery radius is always going to be
// far closer to correct than a CEP misparse in another state.
